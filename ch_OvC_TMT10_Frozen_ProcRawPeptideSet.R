...
fa1psm<-read.table("./ch_10Sept2015_OvC_TMT8_Frozen_HpH_A_med_PSMs.txt", header=TRUE, sep='\t')
fa1pro<-read.table("./ch_10Sept2015_OvC_TMT8_Frozen_HpH_A_med_Proteins.txt", header=TRUE, sep='\t')
fa2psm<-read.table("./ch_10Sept2015_OvC_TMT8_Frozen_HpH_B_med_PSMs.txt", header=TRUE, sep='\t')
fa2pro<-read.table("./ch_10Sept2015_OvC_TMT8_Frozen_HpH_B_med_Proteins.txt", header=TRUE, sep='\t')
fb2psm<-read.table("./ch_18Sept2015_OvC_TMT8_Frozen_HpH_A_PSMs.txt", header=TRUE, sep='\t')
fb2pro<-read.table("./ch_18Sept2015_OvC_TMT8_Frozen_HpH_A_Proteins.txt", header=TRUE, sep='\t')
fb1psm<-read.table("./ch_18Sept2015_OvC_TMT8_Frozen_HpH_A_PSMs.txt", header=TRUE, sep='\t')
fb1pro<-read.table("./ch_18Sept2015_OvC_TMT8_Frozen_HpH_A_Proteins.txt", header=TRUE, sep='\t')
...
processPSM <- function(psmFile, proteinFile, replicate, ... ){	
	...
	pep.a = aggregate(cbind(X126,X127N,X127C,X128N,X128C,X129N,X129C,X130N)~Accession+Gene+Org+Descriptions+Sequence+Confidence,data=pep.r,median,na.action=na.pass,na.rm=TRUE)
	colnames(pep.a) = c('Accession','Gene','Organism','Descriptions','Sequence','Confidence','a1','a2','a3','a4','a5','a6','a7','a8')
	print(dim(pep.a))
	#replace NaN with NA
	pep.a[,7:14] = round(pep.a[,7:14],2)
	pep.a[,7:14][is.na(pep.a[,7:14])]<-NA
	#filter based on S/N
	pep.f1 = subset(pep.a, rowMeans(pep.a[,7:14], na.rm=TRUE)>5)
	#filter based on NA
	pep.f2 = subset(pep.f1, rowSums(is.na(pep.f1[,7:14]))<6)
...
saveRDS(ovfa1.psm,'ch_OvC_Frozen_processedPeptides_fa1.rds')
saveRDS(ovfa2.psm,'ch_OvC_Frozen_processedPeptides_fa2.rds')
saveRDS(ovfb1.psm,'ch_OvC_Frozen_processedPeptides_fb1.rds')
saveRDS(ovfb2.psm,'ch_OvC_Frozen_processedPeptides_fb2.rds')
