# TODO: processing the first sets of ovarian cancer patient samples
# 
# Author: cshughes
###############################################################################
...
processPEP <- function(psmFile,...){
	...
	hu = subset(hu, rowSums(is.na(hu[,c(7:16)]))<7)
...
proTis$fcStd = rowMeans(proTis[,c(4,7)], na.rm=TRUE)
proTis$pvStd = rowMeans(proTis[,c(5,8)], na.rm=TRUE)
proTis$pepStd = rowMeans(proTis[,c(6,9)], na.rm=TRUE)


#make a p-value based on proteins
reps = pro[,c(4,6)]
fit <- lmFit(reps)
fit <- eBayes(fit)
p.value <- fit$p.value
...
write.table(pro,
		'ch_OvC_TMT10plex_proteinSet.txt',
		quote=FALSE,
		sep='\t',
		col.names=TRUE,
		row.names=FALSE)
