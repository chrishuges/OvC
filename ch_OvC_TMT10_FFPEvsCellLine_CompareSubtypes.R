...
prop = merge(pec.q,anno,by='Gene')
#reshape and reannotate dataframe
CL = prop[,c(1,7,8,9,2:6)]
colnames(CL) = c('Gene','Accession','Descriptions','pepNum','logFC','t','score','p-value','pVal')


###############################################################################
#merge the FFPE and Frozen data
###############################################################################
f2 = merge(FFPE, CL, by=c('Gene'))
#plot the data
#make colors
cols = colorRampPalette(c("white", brewer.pal(9,'YlGnBu')[6], brewer.pal(9,'YlOrRd')[9]))
#make a plot
pdf('ch_OvC_TMT10_FFPEvsCellLine_Human_Proteins_FoldChangeScatter.pdf')
reg = lm(f2$logFC.y~f2$logFC.x)
smoothScatter(f2$logFC.x,
		f2$logFC.y,
		main='FFPE vs Cell Lines Fold Change',
		xlab='FFPE',
		ylab='Cell Lines',
		xlim=c(-6,6),
		ylim=c(-6,6),
