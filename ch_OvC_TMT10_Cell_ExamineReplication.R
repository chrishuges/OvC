...
ah1 = readRDS('ch_OvC_CellLine_processedPeptides_fa1.rds')
bh1 = readRDS('ch_OvC_CellLine_processedPeptides_fa2.rds')
#make a function to calculate fold change
doQuant = function(x,...){
	#remove any e-coli peptides
	spikeins = grepl('EColi', x$Organism)
	inSet = x[!spikeins,c(1:4,7:12)]
...
#make a plot
pdf('ch_ch_OvC_TMT10_CellLine_Human_ProteinFoldChange_BiologicalReplicateScatter.pdf')
reg = lm(pSet$logFC.y~pSet$logFC.x)
smoothScatter(pSet$logFC.x,pSet$logFC.y,main='Human Peptide Biological Replicates Cell Lines',
		xlab='repA',
		ylab='repB',
		xlim=c(-6,6),
		ylim=c(-6,6),
